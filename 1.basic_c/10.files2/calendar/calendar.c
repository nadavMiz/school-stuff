...
int LoadCalendar(calendar_t* _destCalendar, char* _fileName)
{
	float beginTime;
	float endTime;
	int roomNum;
	FILE* myFile;
	myFile = fopen(_fileName, "r");
	if(NULL == myFile)
	{
		return 0;
	}
	while(!feof(myFile))
	{
		freed(&beginTime, sizeof(float), 1, myFile);
		freed(&endTime, sizeof(float), 1, myFile);
		freed(&roomNum, sizeof(int), 1, myFile);
		CreateMeeting(beginTime, endTime, roomNum);
	}
	fclose(fileName);
	return 1;
}

int SaveCaledar(calendar_t* fromCalendar, char* fileName){return 1;}

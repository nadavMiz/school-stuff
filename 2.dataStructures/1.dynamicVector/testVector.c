...
void testVectorCreateNormal()
{
	...
	if(!myVec || !(*myVec).m_items || (*myVec).m_originalSize != intial_size || (*myVec).m_size != intial_size || (*myVec).m_nItems != 0 || (*myVec).m_blockSize != block_size)
	{
...
{
	Vector* myVec;
	myVec = VectorCreate(10, 10);
	VectorDestroy(myVec);
	if((*myVec).m_items != NULL)
	{
		printf("FAILED destroy vector normal\n");
	}
	else
	{
		printf("PASSED destroy vector normal\n");
	}
}
...
void testVectorDestroyNULL()
{
	...
	if((*myVec).m_items != NULL)
	{
		printf("FAILED destroy vector NULL\n");
	}
	else
	{
		printf("PASSED destroy vector NULL\n");
	}
}

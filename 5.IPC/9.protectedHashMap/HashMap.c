...
size_t HashMapForEach(const HashMap* _map, KeyValueActionFunction _action, void* _context)
{
	...
	{
		mutexIndex = i % _map->m_numMutexes;
		if(pthread_mutex_lock(&(_map->m_mutexes[mutexIndex])))
		{
			perror("mutex lock");
			break;
		}
		...
		if(pthread_mutex_unlock(&(_map->m_mutexes[mutexIndex])) < 0)
		{
			perror("mutex unlock");
			break;
		}
...
static void DestroyMutexes(pthread_mutex_t* _mutexArr, size_t _NumMutexes)
{
	size_t i;

	for(i = 0; i < _NumMutexes; ++i)
	{
		while(pthread_mutex_destroy(&(_mutexArr[i])) < 0)
		{
			perror("mutex destroy");
			if(EBUSY == errno)
			{
				pthread_mutex_lock(&(_mutexArr[i]));
				pthread_mutex_unlock(&(_mutexArr[i]));
			}
			else
			{
				break;
...
static pthread_mutex_t* CreateMutexes(size_t _numMutex)
{
	size_t i;
	
	pthread_mutex_t* mutexArr = malloc(_numMutex * sizeof(pthread_mutex_t));
	
	for(i = 0; i < _numMutex; ++i)
	{
		if(pthread_mutex_init(&(mutexArr[i]), NULL) < 0)
		{
		 	perror("mutex init\n");
		 	DestroyMutexes(mutexArr, i);
		 	return NULL;
		}
	}
	return mutexArr;
...
static size_t GetNumMutexes(size_t _capacity)
{
	return (_capacity < MAX_MUTEXES) ? _capacity : MAX_MUTEXES;
}
...
HashMap* HashMapCreate(size_t _capacity, HashFunction _hashFunc, EqualityFunction _keysEqualFunc)
{
	...
	hashMap->m_buckets = CreateBuckets(hashMap->m_capacity);
	...
	hashMap->m_numMutexes = GetNumMutexes(hashMap->m_capacity);
	hashMap->m_mutexes = CreateMutexes(hashMap->m_numMutexes);
	if(NULL == hashMap->m_mutexes)
	{
		DestroyBuckets(hashMap->m_buckets, hashMap->m_capacity);
		free(hashMap);
	}
...
MapResult HashMapInsert(HashMap* _map, const void* _key, const void* _value)
{
	Pair* pair;
	ListErrors error;
	size_t index, mutexIndex, hashKey;
	...
	hashKey = _map->m_hashf(_key);
	mutexIndex = hashKey % _map->m_numMutexes;
	index = hashKey % _map->m_capacity;
	
	pthread_mutex_lock(&(_map->m_mutexes[mutexIndex]));
	pair = GetPairBykey(_map, _key);
	pthread_mutex_unlock(&(_map->m_mutexes[mutexIndex]));
...
MapResult HashMapFind(const HashMap* _map, const void* _searchKey, void** _pValue)
{
	...
	mutexIndex = _map->m_hashf(_searchKey) % _map->m_numMutexes;
	
	if(pthread_mutex_lock(&(_map->m_mutexes[mutexIndex])) < 0)
	{
		perror("mutex lock");
		return MAP_MUTEX_ERROR;
	}
...
{
	Pair* targetPair;
	size_t mutexIndex, index;
	int result = 5;
	
	mutexIndex = _map->m_hashf(_key) % _map->m_numMutexes;
	...
	targetPair = GetPairBykey(_map, _key);
	if(NULL == targetPair)
	{
		targetPair = CreatePair(_key, _value);
		ListPushHead(_map->m_buckets[index], targetPair);
		result = MAP_SUCCESS;
	}
	_mergeFunc(targetPair->m_key ,targetPair->m_value ,_context);
...
MapStats* HashMapGetStatistics(const HashMap* _map)
{
	...
	stats->m_collisions = _map->m_collitions;
...
int HashMapActionByKey(const HashMap* _map, const void* _key, KeyValueActionFunction _action, void* _context)
{
	...
	if(NULL != targetPair)
	{
		_action(targetPair->m_key ,targetPair->m_value ,_context);
	}

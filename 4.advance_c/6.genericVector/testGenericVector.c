...
UNIT(TestCreateVectorNormalInt)
	Vector* vec;
...
UNIT(TestCreateVectorNormalPerson)
	Vector* vec;
...
UNIT(TestCreateVectorZeroInput)
	Vector* vec;
	vec = VectorCreate(0, 0);
	ASSERT_THAT(NULL == vec);
END_UNIT

UNIT(TestAddElementsNormalInt(size_t _capacity))
	int i;
	int* elem;
...
UNIT(TestAddElementsNormalPerson(size_t _capacity))
	int i;
	Person* person;
	Result result = PASSED;
...
TEST_SUITE(GenericVector test)
	TEST(TestCreateVectorNormalInt)
	TEST(TestCreateVectorNormalPerson)
	TEST(TestCreateVectorZeroInput)
END_SUITE

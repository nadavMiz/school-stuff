...
void ShmpProtocol::GetTopic(std::stringstream& _msgStream, ProtocolMsgPtr _msg) const
{
	if(_msgStream.eof())
	{
...
void ShmpProtocol::GetValues(std::stringstream& _msgStream, ProtocolMsgPtr _msg) const
{
	std::string value;
...
ProtocolMsgPtr ShmpProtocol::ParseMessage(char* _data, std::size_t _length)
{
	std::stringstream msgStream(_data);
	ValidateProtocol(msgStream);
	
	ProtocolMsgPtr result(new ProtocolMsg);
